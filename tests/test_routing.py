"""Guards on the skill-routing check.

Routing is the measured claim this pack actually stands on: does a realistic phrasing
reach the skill that owns it, and do any two descriptions collide? These tests mirror
the CI step so a description edit that breaks routing fails locally, before a PR.

They also enforce coverage — every skill ships trigger cases, and every negative case
names the skill that *should* win, so an assertion can never pass vacuously by matching
nothing at all.
"""
...
SCRIPTS = ROOT / "benchmarks" / "routing"
CASES = ROOT / "benchmarks" / "routing" / "cases"
SKILLS = ROOT / "skills"
...
def test_every_skill_has_a_trigger_case_file():
    ...
    assert not missing, (
        f"no benchmarks/routing/cases/<name>.json for: {missing}. Every skill ships "
